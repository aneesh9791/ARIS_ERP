-- Migration 013: Bill Printing Configuration and Templates
-- Source: /Volumes/DATA HD/ARIS_ERP/New_ERP_Project/database/bill-printing-schema.sql

-- BILL CONFIGURATION TABLE
CREATE TABLE IF NOT EXISTS bill_configuration (
...
-- CENTER LOGOS TABLE (position as column name is fine in CREATE TABLE)
CREATE TABLE IF NOT EXISTS center_logos (
    id             SERIAL PRIMARY KEY,
    center_id      INTEGER REFERENCES centers(id),
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_bill_configuration_center_id  ON bill_configuration(center_id);
CREATE INDEX IF NOT EXISTS idx_bill_configuration_is_default ON bill_configuration(is_default);
CREATE INDEX IF NOT EXISTS idx_center_logos_center_id        ON center_logos(center_id);
CREATE INDEX IF NOT EXISTS idx_center_logos_type             ON center_logos(logo_type);
CREATE INDEX IF NOT EXISTS idx_print_jobs_bill_id            ON print_jobs(bill_id);
...
-- Sample data: bill configurations
INSERT INTO bill_configuration (
    ...
    logo_position, font_size, paper_size, orientation,
    margin_top, margin_bottom, margin_left, margin_right, is_default
) VALUES
(1, 'Standard Template',  'MEDICAL IMAGING CENTER',
 ...
 true, true, true, true, true, true, true, true, 'CENTER', 12, 'A4', 'PORTRAIT', 1.0, 1.0, 1.0, 1.0, true),
(1, 'Simple Template',    'DIAGNOSTIC CENTER',
 'We appreciate your business',
 E'1. Payment due within 15 days\n2. No returns or exchanges',
 true, true, true, false, false, false, false, false, 'LEFT', 10, 'A4', 'PORTRAIT', 1.5, 1.5, 1.5, 1.5, false),
(1, 'Detailed Template',  'ADVANCED MEDICAL IMAGING',
 'Your health is our priority',
 E'1. Payment due within 45 days\n2. Late payment charges @ 24% per annum\n3. All disputes subject to Kerala High Court jurisdiction\n4. Comprehensive insurance coverage available\n5. Emergency services available 24/7',
 true, true, true, true, true, true, true, true, 'RIGHT', 14, 'A4', 'LANDSCAPE', 0.5, 0.5, 0.5, 0.5, false);

-- Sample data: bill templates
INSERT INTO bill_templates (template_name, template_type, html_content, css_content, variables) VALUES
...
-- Sample data: center logos
INSERT INTO center_logos (center_id, logo_name, logo_path, logo_type, position, file_size, file_extension) VALUES
