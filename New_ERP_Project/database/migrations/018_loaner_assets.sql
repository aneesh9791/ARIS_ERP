-- Migration 018: Loaner Asset Tracking System
-- Source: /Volumes/DATA HD/ARIS_ERP/New_ERP_Project/backend/migrations/loaner_asset_tracking.sql
-- Fix applied: DATEDIFF(CURRENT_DATE, laa.expected_return_date) in check_overdue_loaner_assets
--   replaced with (CURRENT_DATE - laa.expected_return_date)::INTEGER for PostgreSQL.
-- Fix applied: assign_loaner_asset INSERT references agreement_required column which
--   does not exist in loaner_asset_assignments — the column is called agreement_signed.
--   Changed column name to agreement_signed in the INSERT.

-- Extend expense_items_master to support loaner asset association
...
ALTER TABLE expense_items_master ADD COLUMN IF NOT EXISTS loaner_asset_condition_required VARCHAR(20) DEFAULT 'GOOD';
...
-- LOANER ASSET ASSIGNMENTS TABLE
...
-- LOANER ASSET MAINTENANCE TABLE
...
-- LOANER ASSET PERFORMANCE TABLE
CREATE TABLE IF NOT EXISTS loaner_asset_performance (
    ...
    consumable_efficiency   DECIMAL(5,2),
...
SELECT
    ...
    COALESCE(AVG(lap.user_satisfaction), 0)                     AS avg_user_satisfaction,
    CASE WHEN am.id IS NOT NULL THEN true ELSE false END        AS has_loaner_asset
...
GROUP BY eim.id, eim.item_code, eim.item_name, eim.current_stock, eim.minimum_stock_level,
         am.id, am.asset_code, am.asset_name, am.asset_type, am.status;
...
SELECT
    ...
    COALESCE(SUM(laa.consumable_quantity_given), 0)                                                                 AS total_consumables_given,
    COALESCE(SUM(CASE WHEN laa.assignment_date >= CURRENT_DATE - INTERVAL '30 days'
                      THEN laa.consumable_quantity_given ELSE 0 END), 0)                                            AS consumables_last_30_days,
    COALESCE(AVG(lap.user_satisfaction), 0)                                                                         AS avg_user_satisfaction,
...
-- Fix: INSERT used column name 'agreement_required' — corrected to 'agreement_signed'
...
BEGIN
    ...
    IF v_consumable_stock < p_consumable_quantity_given THEN
        RAISE EXCEPTION 'Insufficient consumable stock. Available: %, Required: %',
            v_consumable_stock, p_consumable_quantity_given;
    END IF;
    ...
    INSERT INTO loaner_asset_assignments (
        ...
        condition_at_assignment, agreement_signed, notes, assigned_by
    ) VALUES (
        ...
        'GOOD', COALESCE(v_agreement_required, false), p_notes, p_assigned_by
    ...
    INSERT INTO loaner_asset_movements (
        ...
        p_assigned_by,
        'Assigned against consumable: ' || p_consumable_quantity_given || ' units'
    );
    ...
    INSERT INTO expense_stock_movements (
        ...
        -p_consumable_quantity_given,
        v_consumable_stock - p_consumable_quantity_given,
        p_consumable_unit_cost,
        p_consumable_quantity_given * p_consumable_unit_cost,
        p_assigned_by,
        'Consumables given for loaner asset assignment'
    );
...
BEGIN
    ...
    INSERT INTO loaner_asset_movements (
        ...
        p_returned_by,
        'Asset returned with condition: ' || p_condition_at_return
    );
...
-- Fix: DATEDIFF(CURRENT_DATE, laa.expected_return_date) →
--      (CURRENT_DATE - laa.expected_return_date)::INTEGER
...
BEGIN
    ...
    SELECT
        ...
        (CURRENT_DATE - laa.expected_return_date)::INTEGER AS days_overdue,
        laa.deposit_amount,
...
RETURNS TRIGGER AS $$
BEGIN
    -- Placeholder for future periodic overdue checking automation
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Data updates: flag consumable categories as supporting loaner assets
UPDATE expense_items_master
...
-- Sample loaner association (only runs if FILM-DR-100 item exists and a film printer asset exists)
UPDATE expense_items_master
