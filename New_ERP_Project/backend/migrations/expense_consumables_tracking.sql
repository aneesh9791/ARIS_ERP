-- Expense Consumables and Stationaries Tracking System
-- Links expense items with vendor master and tracks consumption for timely refilling
...
-- Create expense items master table
CREATE TABLE IF NOT EXISTS expense_items_master (
    ...
    standard_package_size INTEGER DEFAULT 1, -- Number of units per package
    ...
    days_of_supply INTEGER DEFAULT 30, -- Days current stock will last
...
-- Create vendor-expense item linkage table
CREATE TABLE IF NOT EXISTS expense_item_vendors (
    ...
    vendor_sku VARCHAR(50), -- Vendor's SKU/part number
    vendor_item_name VARCHAR(100),
    unit_price DECIMAL(10,2),
    bulk_pricing JSONB, -- Tiered pricing for bulk orders
...
-- Create expense consumption tracking table
CREATE TABLE IF NOT EXISTS expense_consumption (
    ...
    reference_document VARCHAR(50), -- Work order, patient ID, etc.
    notes TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    active BOOLEAN DEFAULT true
...
-- Create expense purchase orders table
CREATE TABLE IF NOT EXISTS expense_purchase_orders (
    ...
    order_status VARCHAR(20) DEFAULT 'PENDING', -- PENDING, APPROVED, ORDERED, PARTIAL_DELIVERED, DELIVERED, CANCELLED
...
-- Create expense purchase order items table
...
-- Create expense stock movements table
CREATE TABLE IF NOT EXISTS expense_stock_movements (
    ...
    quantity_change DECIMAL(12,2) NOT NULL, -- Positive for additions, negative for reductions
...
-- Create expense stock alerts table
...
-- Create expense consumption forecasts table
CREATE TABLE IF NOT EXISTS expense_consumption_forecasts (
    ...
    factors_considered TEXT, -- Seasonality, special events, etc.
...
SELECT 
    ...
    COUNT(DISTINCT ec.id) as total_consumptions
...
BEGIN
    ...
    -- Get unit cost if it's a purchase
    IF p_movement_type = 'PURCHASE' THEN
        ...
        AND epo.purchase_order_id = p_reference_id
    ...
    INSERT INTO expense_stock_movements (
        ...
        v_new_stock, v_unit_cost, COALESCE(v_unit_cost, 0) * ABS(p_quantity_change),
    ...
    ELSIF v_new_stock <= (SELECT minimum_stock_level FROM expense_items_master WHERE id = p_expense_item_id) THEN
        INSERT INTO expense_stock_alerts (
            ...
            v_new_stock, (SELECT minimum_stock_level FROM expense_items_master WHERE id = p_expense_item_id),
            0, (SELECT reorder_quantity FROM expense_items_master WHERE id = p_expense_item_id),
            (SELECT reorder_quantity * eiv.unit_price FROM expense_items_master eim JOIN expense_item_vendors eiv ON eim.id = eiv.expense_item_id WHERE eim.id = p_expense_item_id AND eiv.preferred_vendor = true),
...
-- Function to calculate average monthly consumption
CREATE OR REPLACE FUNCTION calculate_average_consumption(p_expense_item_id INTEGER, p_months INTEGER DEFAULT 3)
RETURNS DECIMAL AS $$
DECLARE
...
BEGIN
    ...
    WHERE eim.active = true
    AND (p_center_id IS NULL OR eim.center_id = p_center_id)
...
-- Trigger to automatically update consumption when records are added
CREATE OR REPLACE FUNCTION trigger_update_consumption_stats()
RETURNS TRIGGER AS $$
BEGIN
    -- Update average monthly consumption for the item
...
-- Apply trigger for consumption updates
...
('PEN-BALL', 'Ball Pen Blue', 2, 'Blue ball point pen', 'PACKETS', 100, 20, 200, 50, 60, 'Stationery Store'),
('PAPER-A4', 'A4 Paper', 2, 'A4 size white paper 500 sheets', 'PACKETS', 20, 5, 50, 10, 15, 'Stationery Store'),
('HAND-SANITIZER', 'Hand Sanitizer 500ml', 3, 'Alcohol-based hand sanitizer', 'BOTTLES', 30, 10, 100, 20, 25, 'Reception'),
('DISINFECTANT', 'Disinfectant Liquid 1L', 3, 'Multi-surface disinfectant', 'BOTTLES', 15, 5, 50, 10, 12, 'Housekeeping')
ON CONFLICT (item_code) DO NOTHING;

-- Link items to existing vendors (assuming vendor IDs from asset_vendors table)
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_categories TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_items_master TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_item_vendors TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_consumption TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_purchase_orders TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_purchase_order_items TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_stock_movements TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_stock_alerts TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON expense_consumption_forecasts TO your_db_user;

-- Grant select on views
-- GRANT SELECT ON expense_items_summary TO your_db_user;
-- GRANT SELECT ON expense_consumption_analysis TO your_db_user;
-- GRANT SELECT ON expense_vendor_performance TO your_db_user;

COMMENT ON TABLE expense_categories IS 'Categories for expense items like consumables, stationaries, equipment, services';
COMMENT ON TABLE expense_items_master IS 'Master table for all expense items with stock tracking';
COMMENT ON TABLE expense_item_vendors IS 'Links expense items with vendors for pricing and procurement';
COMMENT ON TABLE expense_consumption IS 'Tracks consumption of expense items by departments and users';
COMMENT ON TABLE expense_purchase_orders IS 'Purchase orders for expense items with vendor details';
COMMENT ON TABLE expense_stock_movements IS 'Complete audit trail of all stock movements';
COMMENT ON TABLE expense_stock_alerts IS 'Automated alerts for low stock, expiry, and reorder recommendations';
COMMENT ON TABLE expense_consumption_forecasts IS 'Consumption forecasting for better inventory planning';
