...
-- Following industry-standard accounting practices

-- Accounting Entries Table (Double-Entry Bookkeeping)
...
-- Accounting Bills Table (Enhanced Billing)
...
CHECK (billing_status IN ('DRAFT', 'PENDING', 'POSTED', 'PARTIALLY_PAID', 'FULLY_PAID', 'OVERPAID', 'VOIDED', 'WRITTEN_OFF', 'DISPUTED', 'SENT_TO_COLLECTION'));

ALTER TABLE accounting_bills 
ADD CONSTRAINT chk_payment_status 
CHECK (payment_status IN ('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'CANCELLED', 'REFUNDED', 'PARTIALLY_REFUNDED', 'CHARGEBACK', 'REVERSED', 'HELD'));
...
ALTER TABLE accounting_bills 
ADD CONSTRAINT chk_balance_amount 
CHECK (balance_amount >= 0);

ALTER TABLE accounting_payments 
ADD CONSTRAINT chk_payment_method 
CHECK (payment_method IN ('CASH', 'CHECK', 'WIRE_TRANSFER', 'ACH_TRANSFER', 'CREDIT_CARD', 'DEBIT_CARD', 'BANK_TRANSFER', 'UPI', 'NET_BANKING', 'MOBILE_WALLET', 'CRYPTOCURRENCY', 'INSURANCE', 'CORPORATE', 'GOVERNMENT', 'COMBINED'));

ALTER TABLE accounting_payments 
ADD CONSTRAINT chk_payment_status_payment 
CHECK (payment_status IN ('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'CANCELLED', 'REFUNDED', 'PARTIALLY_REFUNDED', 'CHARGEBACK', 'REVERSED', 'HELD'));
...
CREATE TRIGGER trigger_accounting_bill_timestamp
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_accounting_bill_timestamp();
...
CREATE TRIGGER trigger_accounting_payment_timestamp
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_accounting_payment_timestamp();
...
(EXTRACT(YEAR FROM CURRENT_DATE), 2, 'Q2 - Jul-Sep',
    DATE_TRUNC('quarter', CURRENT_DATE),
    DATE_TRUNC('quarter', CURRENT_DATE) + INTERVAL '3 months' - INTERVAL '1 day'),
(EXTRACT(YEAR FROM CURRENT_DATE), 3, 'Q3 - Oct-Dec',
    DATE_TRUNC('quarter', CURRENT_DATE + INTERVAL '3 months'),
    DATE_TRUNC('quarter', CURRENT_DATE + INTERVAL '3 months') + INTERVAL '3 months' - INTERVAL '1 day'),
(EXTRACT(YEAR FROM CURRENT_DATE), 4, 'Q4 - Jan-Mar',
