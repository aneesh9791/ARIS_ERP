-- User-Friendly Settings and Configuration System
-- Pre-configured settings with dropdown selections for easy use
...
-- Create department settings table
...
-- Insert system settings with pre-configured values
...
-- Insert dropdown options for various fields
...
-- Insert form configurations for easy-to-use forms
...
-- Insert quick actions for easy navigation
...
('asset_maintenance_due', 'EMAIL', 'ASSET', 'Asset Maintenance Due', 'Dear {user_name},\n\nThis is a reminder that the asset "{asset_name}" ({asset_code}) is due for maintenance on {maintenance_date}.\n\nAsset Details:\n- Name: {asset_name}\n- Code: {asset_code}\n- Location: {location}\n- Last Maintenance: {last_maintenance_date}\n\nPlease schedule the maintenance at your earliest convenience.\n\nRegards,\nARIS Healthcare System', '{"user_name": "User name", "asset_name": "Asset name", "asset_code": "Asset code", "maintenance_date": "Maintenance date", "location": "Location", "last_maintenance_date": "Last maintenance date"}'),

('low_stock_alert', 'EMAIL', 'INVENTORY', 'Low Stock Alert', 'Low Stock Alert - {item_name}', 'Dear {user_name},\n\nThe following item is running low on stock:\n\nItem: {item_name}\nCurrent Stock: {current_stock} {unit_of_measure}\nMinimum Stock: {minimum_stock} {unit_of_measure}\nRecommended Order: {reorder_quantity} {unit_of_measure}\n\nPlease reorder this item soon to avoid stockouts.\n\nRegards,\nARIS Healthcare System', '{"user_name": "User name", "item_name": "Item name", "current_stock": "Current stock", "unit_of_measure": "Unit of measure", "minimum_stock": "Minimum stock", "reorder_quantity": "Reorder quantity"}'),

('loaner_asset_overdue', 'EMAIL', 'ASSET', 'Loaner Asset Overdue', 'Overdue Loaner Asset - {asset_name}', 'Dear {user_name},\n\nThe following loaner asset is overdue for return:\n\nAsset: {asset_name}\nAssigned to: {assigned_to}\nDue Date: {due_date}\nDays Overdue: {days_overdue}\nDeposit Amount: ₹{deposit_amount}\n\nPlease follow up with the assignee for the return of this asset.\n\nRegards,\nARIS Healthcare System', '{"user_name": "User name", "asset_name": "Asset name", "assigned_to": "Assigned to", "due_date": "Due date", "days_overdue": "Days overdue", "deposit_amount": "Deposit amount"}'),

('appointment_reminder', 'SMS', 'PATIENT', 'Appointment Reminder', 'Dear {patient_name}, this is a reminder for your appointment on {appointment_date} at {appointment_time} with {doctor_name}. Please arrive 15 minutes early. For any changes, call {phone_number}.', '{"patient_name": "Patient name", "appointment_date": "Appointment date", "appointment_time": "Appointment time", "doctor_name": "Doctor name", "phone_number": "Phone number"}'),

('journal_entry_approval', 'EMAIL', 'FINANCIAL', 'Journal Entry Approval Required', 'Journal Entry Approval Required - {entry_number}', 'Dear {approver_name},\n\nA journal entry requires your approval:\n\nEntry Number: {entry_number}\nDate: {entry_date}\nAmount: ₹{total_amount}\nDescription: {description}\nCreated by: {created_by}\n\nPlease review and approve at your earliest convenience.\n\nRegards,\nARIS Healthcare System', '{"approver_name": "Approver name", "entry_number": "Entry number", "entry_date": "Entry date", "total_amount": "Total amount", "description": "Description", "created_by": "Created by"}')
ON CONFLICT (template_name) DO NOTHING;

-- Create views for easy access to settings
...
SELECT 
    ...
    ss.sort_order,
    CASE 
        WHEN ss.setting_type = 'SELECT' THEN json_extract_path_text(ss.display_options, 'options')
...
SELECT 
    ...
    do.option_value,
    do.option_label,
    do.option_description,
    do.parent_option_value,
    do.sort_order,
    do.is_default,
    CASE 
        WHEN do.parent_option_value IS NOT NULL THEN 
            (SELECT option_label FROM dropdown_options WHERE option_group = do.option_group AND option_value = do.parent_option_value)
        ELSE NULL
    END as parent_label
FROM dropdown_options do
WHERE do.is_active = true
ORDER BY do.option_group, do.parent_option_value NULLS FIRST, do.sort_order;

-- Grant permissions (adjust based on your database user)
-- GRANT SELECT, INSERT, UPDATE, DELETE ON system_settings TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON user_preferences TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON center_settings TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON department_settings TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON dropdown_options TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON form_configurations TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON quick_actions TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON dashboard_widgets TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON notification_templates TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON workflow_configurations TO your_db_user;

-- Grant select on views
-- GRANT SELECT ON user_friendly_settings TO your_db_user;
-- GRANT SELECT ON dropdown_options_view TO your_db_user;
