-- Comprehensive Chart of Accounts System
-- Creates different chart of accounts for various transactions
-- Supports asset management, expense tracking, loaner assets, and general accounting
...
CREATE TABLE IF NOT EXISTS chart_of_accounts (
    ...
    account_category VARCHAR(30) NOT NULL, -- Current Assets, Fixed Assets, Current Liabilities, etc.
    account_subcategory VARCHAR(50),
    parent_account_id INTEGER REFERENCES chart_of_accounts(id),
    account_level INTEGER DEFAULT 1, -- 1 for main accounts, 2 for sub-accounts, etc.
    ...
    is_consolidated BOOLEAN DEFAULT false, -- For summary accounts
...
-- Create account groups for better organization
...
CREATE TABLE IF NOT EXISTS journal_entries (
    ...
    reference_type VARCHAR(30), -- INVOICE, PAYMENT, RECEIPT, PURCHASE_ORDER, EXPENSE, ASSET_PURCHASE, etc.
...
-- Create projects table (for project-based accounting)
CREATE TABLE IF NOT EXISTS projects (
    id SERIAL PRIMARY KEY,
    project_code VARCHAR(20) NOT NULL UNIQUE,
    project_name VARCHAR(100) NOT NULL,
    project_type VARCHAR(20), -- CAPITAL, REVENUE, RESEARCH, MAINTENANCE
...
CREATE TABLE IF NOT EXISTS tax_configuration (
    ...
    tax_type VARCHAR(20) NOT NULL, -- GST, TDS, VAT, SERVICE_TAX, etc.
    tax_rate DECIMAL(5,2) NOT NULL,
    applicable_on VARCHAR(20), -- PURCHASE, SALE, BOTH
    input_tax_credit_allowed BOOLEAN DEFAULT false,
...
-- Create functions for automated accounting
CREATE OR REPLACE FUNCTION create_journal_entry(
    ...
    p_description TEXT,
    p_lines JSON, -- JSON array of line items with account_id, debit_amount, credit_amount, description
...
BEGIN
    ...
    FOR v_line_index IN 0..json_array_length(p_lines)-1 LOOP
        ...
        INSERT INTO journal_entry_lines (
            ...
            v_entry_id, v_line_index + 1, (v_line_record->>'account_id')::INTEGER,
...
BEGIN
    -- Check if entry exists and is approved
    ...
    -- Update account balances (already done in create function)
    -- Additional posting logic can be added here
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON chart_of_accounts TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON account_groups TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON transaction_types TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON journal_entries TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON journal_entry_lines TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON cost_centers TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON projects TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON financial_periods TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON trial_balance TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON balance_sheet TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON profit_loss TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON account_reconciliation TO your_db_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON tax_configuration TO your_db_user;

-- Grant select on views
-- GRANT SELECT ON account_hierarchy TO your_db_user;
-- GRANT SELECT ON transaction_summary TO your_db_user;
-- GRANT SELECT ON account_balance_summary TO your_db_user;

COMMENT ON TABLE chart_of_accounts IS 'Master chart of accounts for all financial transactions';
COMMENT ON TABLE account_groups IS 'Groups for organizing chart of accounts';
COMMENT ON TABLE transaction_types IS 'Types of financial transactions with default accounts';
COMMENT ON TABLE journal_entries IS 'Main journal entries for all transactions';
COMMENT ON TABLE journal_entry_lines IS 'Line items for journal entries with debit/credit amounts';
COMMENT ON TABLE cost_centers IS 'Cost centers for department-wise expense tracking';
COMMENT ON TABLE projects IS 'Projects for project-based accounting';
COMMENT ON TABLE financial_periods IS 'Financial periods for reporting';
COMMENT ON TABLE trial_balance IS 'Trial balance reports for financial periods';
COMMENT ON TABLE balance_sheet IS 'Balance sheet reports';
COMMENT ON TABLE profit_loss IS 'Profit and loss statements';
COMMENT ON TABLE account_reconciliation IS 'Account reconciliation records';
COMMENT ON TABLE tax_configuration IS 'Tax rates and configuration';
